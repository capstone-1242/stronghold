        ...
        @if ($memorialImage)
            <form action="{{ route('memorial-images.update', $memorialImage->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <div>
                        @php
                            $isCdn = strpos($memorialImage->filename, 'http') === 0;
                            $imageSrc = $isCdn ? $memorialImage->filename : Storage::url($memorialImage->filename);
                        @endphp
                        <img src="{{ $imageSrc }}" alt="{{ $memorialImage->description }}" class="edit-image max-w-full rounded-xl shadow">
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <input type="submit" value="Update Memorial Image" class="cursor-pointer bg-sky-900 text-white p-2 rounded-xl hover:bg-sky-600 w-full">
                </div>
            </form>
        @endif
