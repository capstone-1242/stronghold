<x-admin-layout :title="'Edit a Testimonial'">
   <section class="p-6 md:p-12 container">
        ...
        <form method="GET" action="{{ route('auth.edit.testimonial') }}" class="mb-6">
            ...
            <select id="select-testimonial" name="testimonial_video_id" class="border border-gray-800 p-2 w-full rounded-xl bg-white" onchange="this.form.submit()">
        ...
        @if ($testimonial_video)
            <form action="{{ route('testimonial.update', $testimonial_video->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $testimonial_video->title) }}" class="border border-gray-800 p-2 w-full rounded-xl bg-white" required>
                    <small>Provide a title for the testimonial.</small>
                </div>
    
                <div class="mb-6">
                    <label for="description" class="block w-full">Description</label>
                    <textarea id="description" name="description" class="border border-gray-800 p-2 w-full rounded-xl bg-white" required>{{ old('description', $testimonial_video->description) }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <input type="url" id="url" name="url" value="{{ old('url', $testimonial_video->url) }}" class="border border-gray-800 p-2 w-full rounded-xl bg-white" required>
                ...
                <div class="mb-6">
                    <input type="submit" value="Update Testimonial" class="cursor-pointer bg-sky-900 text-white p-2 rounded-xl hover:bg-sky-600 w-full">
                </div>
            </form>
        @endif
