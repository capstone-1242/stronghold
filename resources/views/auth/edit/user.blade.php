<x-admin-layout>
    ...
    <form method="GET" action="{{ route('auth.edit.user') }}" class="mb-6">
        <div class="mb-6">
            ...
            <select id="select-user" name="user_id" class="border border-gray-800 p-2 w-full rounded-xl" onchange="this.form.submit()">
    ...
    @if ($user)
        <form action="{{ route('user.update', $user->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="first_name" name="first_name" value="{{ old('first_name', $user->first_name) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Enter the user's first name.</small>
            </div>

            <div class="mb-6">
                <label for="last_name" class="block w-full">Last Name</label>
                <input type="text" id="last_name" name="last_name" value="{{ old('last_name', $user->last_name) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Enter the user's last name.</small>
            </div>

            <div class="mb-6">
                <label for="email" class="block w-full">Email</label>
                <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Provide a valid email address for the user.</small>
            </div>

            <div class="mb-6">
                <label for="role" class="block w-full">Role</label>
                <select id="role" name="role" class="border border-gray-800 p-2 w-full rounded-xl" required>
            ...
            <div class="mb-6">
                ...
                <input type="password" id="password" name="password" class="border border-gray-800 p-2 w-full rounded-xl" autocomplete="new-password">
            ...
            <div class="mb-6">
                ...
                <input type="password" id="password_confirmation" name="password_confirmation" class="border border-gray-800 p-2 w-full rounded-xl">
