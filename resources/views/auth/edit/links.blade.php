<x-admin-layout>
    ...
    <form method="GET" action="{{ route('auth.edit.links') }}" class="mb-6">
        ...
        <select id="select-link" class="border border-gray-800 p-2 w-full rounded-xl" name="link_id" onchange="this.form.submit()">
    ...
    @if ($link)
        <form action="{{ route('links.update', $link->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="title" name="title" class="border border-gray-800 p-2 w-full rounded-xl" value="{{ old('title', $link->title) }}" required>
                <small>Please enter the title of the link.</small>
            </div>

            <div class="mb-6">
                <label for="url" class="block w-full">URL</label>
                <input type="url" id="url" name="url" class="border border-gray-800 p-2 w-full rounded-xl" value="{{ old('url', $link->url) }}" required>
            ...
            <div>
                <input type="submit" value="Update Link" class="bg-sky-900 text-white p-2 rounded-xl hover:bg-sky-600 w-full">
            </div>
        </form>
    @endif
