<x-admin-layout>
    ...
    <form method="GET" action="{{ route('auth.edit.resource') }}" class="mb-6">
        ...
        <select id="select-resource" name="resource_id" onchange="this.form.submit()" class="border border-gray-800 p-2 w-full rounded-xl">
    ...
    @if ($resource)
        <form action="{{ route('resource.update', $resource->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $resource->title) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Please provide a title for the resource.</small>
            </div>

            <div class="mb-6">
                <label for="description" class="block w-full">Description</label>
                <textarea id="description" name="description" class="border border-gray-800 p-2 w-full rounded-xl" required>{{ old('description', $resource->description) }}</textarea>
                <small>Provide a description of the resource.</small>
            </div>

            <div class="mb-6">
                <label for="url" class="block w-full">URL</label>
                <input type="url" id="url" name="url" value="{{ old('url', $resource->url) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Please provide the URL for the resource.</small>
            </div>

            <div class="mb-6">
                <label for="number" class="block w-full">Phone Number</label>
                <input type="text" id="number" name="number" value="{{ old('number', $resource->number) }}" class="border border-gray-800 p-2 w-full rounded-xl">
            ...
            <div class="mb-6">
                ...
                <select id="tag_id" name="tag_id" class="border border-gray-800 p-2 w-full rounded-xl">
