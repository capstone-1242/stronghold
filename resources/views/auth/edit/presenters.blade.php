<x-admin-layout>
    ...
    <form method="GET" action="{{ route('auth.edit.presenters') }}" class="mb-6">
        ...
        <select id="select-author" name="author_id" class="border border-gray-800 p-2 w-full rounded-xl" onchange="this.form.submit()">
    ...
    @if ($author)
        <form action="{{ route('presenters.update', $author->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="first_name" name="first_name" value="{{ old('first_name', $author->first_name) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Please enter the first name of the presenter.</small>
            </div>

            <div class="mb-6">
                <label for="last_name" class="block w-full">Last Name</label>
                <input type="text" id="last_name" name="last_name" value="{{ old('last_name', $author->last_name) }}" class="border border-gray-800 p-2 w-full rounded-xl" required>
                <small>Please enter the last name of the presenter.</small>
            </div>

            <div class="mb-6">
                <label for="description" class="block w-full">Description</label>
                <textarea id="description" name="description" class="border border-gray-800 p-2 w-full rounded-xl" required>{{ old('description', $author->description) }}</textarea>
