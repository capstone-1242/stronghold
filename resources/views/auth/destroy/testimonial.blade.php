                    ...
                    <tr class="border-b">
                        ...
                        <td class="p-4">
                            <form action="{{ route('auth.destroy.testimonial.delete', $testimonial->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <input type="submit" value="Delete" class="bg-red-500 text-white px-4 py-2 rounded-xl hover:bg-red-700 cursor-pointer" onclick="return confirmDelete('{{ $testimonial->title }}')" />
                            </form>
                        </td>
                    </tr>
                @endforeach
